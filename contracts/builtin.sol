...
library Builtin {
    ...
    function getAuthority() external pure returns (Authority) {
        return Authority(uint72(bytes9("Authority")));
    }

    /// @return the instance of the contract "Energy".
    function getEnergy() external pure returns (Energy) {
        return Energy(uint48(bytes6("Energy")));
    }

    /// @return the instance of the contract "Extension".
    function getExtension() external pure returns (Extension) {
        return Extension(uint72(bytes9("Extension")));
    }

    /// @return the instance of the contract "Params".
    function getParams() external pure returns (Params) {
        return Params(uint48(bytes6("Params")));
    }

    /// @return the instance of the contract "Executor".
    function getExecutor() external pure returns (Executor) {
        return Executor(uint64(bytes8("Executor")));
    }

    Energy constant energy = Energy(uint48(bytes6("Energy")));
    ...
    Prototype constant prototype = Prototype(uint72(bytes9("Prototype")));
